...
class ResearchManager:
    ...
    async def run(self, query: str, recipient_email: str | None = None):
    ...
    async def plan_searches(self, query: str) -> WebSearchPlan:
        ...
        result = await rate_limiter.apply(
            Runner.run,
            planner_agent,
            f"Query: {query}",
        )
    ...
    #swapped parallel searches for sequential searches
    async def perform_searches(self, search_plan: WebSearchPlan) -> list[str]:
        """ Perform the searches to perform for the query """
        print("Searching...")
        results = []
        ...
        for i, item in enumerate(search_plan.searches):
            print(f"Wyszukiwanie {i+1}/{len(search_plan.searches)}...")
            result = await self.search(item)
            if result is not None:
                results.append(result)
            print(f"Ukończono wyszukiwanie {i+1}/{len(search_plan.searches)}")
